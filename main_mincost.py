...
plt.ylim(-sp_y.max() + 1, 0)
plt.subplot(313)
#plt.imshow(minimum_cost_PU_sparse(x.ravel(), y.ravel(), psi.ravel()).reshape(*psi.shape), cmap='gray')
plt.scatter(sp_x, -sp_y, c=minimum_cost_PU_sparse(sp_x, sp_y, sp_psi, capacity=1), cmap='gray', s=0.05)
plt.xlim(0, sp_x.max() - 1)
plt.ylim(-sp_y.max() + 1, 0)
plt.show()
